...
export const ProjectView = () => {
  ...
  const {data: project, refetch: projectReFetch, isLoading: projectIsLoading} = useQuery(['project', id], () => {
    return Axios.get(`projects/${id?.id}`).then(res => {
  ...
  return (
    <main>
      ...
      <section className="flex gap-6 border border-blackFactory px-6 py-4">
        <section className="max-w-[640px] object-contain"><img src={`${imgUrl}/${project?.image}`} alt=""/></section>
        <section className='flex-2 justify-center flex flex-col w-full gap-y-3'>

          <section className='flex flex-col gap-3'>
            <div className="flex items-center gap-x-2">
              <div className="w-[56px] h-[56px] border rounded-[50%]">
                <img src={`https://robohash.org/${project?.user.username}`} alt=""/>
              </div>
              <div>
          ...
          <div className="w-[80%]">{project?.description} Project's Description</div>
          ...
          <section className="flex py-4 justify-between border-y-2 border-[#D9D9D9]">
            ...
            <div className='flex flex-col items-center'>
              ...
              <div className='flex '>{project?.project_deadline}</div>
            </div>
          </section>

...
export const Replier = (props) => {
  ...
  const ImageContent = () => {
    return (
      <img onClick={(e) => {
        e.stopPropagation();
        setHandleExpand(1)}} className="md:max-w-[250px] max-w-[156px] object-contain" src={`http://127.0.0.1:8000/${image}`} alt=""/>
    );
  }
              ...
              <div className={`${image ? 'p-0' : 'p-3'} ${messageContent ? 'bg-gray-300' : 'bg-transparent'} rounded-r-lg rounded-bl-lg`}>
                ...
                {image &&
                  <img onClick={(e) => {
                    e.stopPropagation();
                    setHandleExpand(1)}}
                       className="cursor-pointer md:max-w-[250px] max-w-[156px] object-contain" src={`http://127.0.0.1:8000/${image}`} alt=""/>}
                <ImageExpand open={handleExpand} content={`http://127.0.0.1:8000/${image}`} setOpen={setHandleExpand}/>
              </div>

...
export const SearchContent = () => {
  ...
  const {searchInput, setSearchInput, projects, modalOpen, setModalOpen} = useProjectContext()
      ...
      <main className="min-w-[1920px]:px-48 xl:px-24 md:px-12
        ...
        <section className="flex justify-between">
          <section className="flex flex-col h-fit items-center pr-24 border-r-2 border-grayFactory gap-4">
            <button className={`${searchTab === 'all' && 'text-redBase'}`} onClick={() => setSearchTab('all')}>All
            </button>
            <button className={`${searchTab === 'users' && 'text-redBase'} whitespace-nowrap`}
                    onClick={() => setSearchTab('users')}>Users
            </button>
            <button className={`${searchTab === 'projects' && 'text-redBase'} whitespace-nowrap`}
                    onClick={() => setSearchTab('projects')}>Projects
            </button>
            ...
            {searchTab === 'all' &&
              <>
                ...
                {filteredProject?.slice(0, 5).map((project, key) => {
                  return (
                    <ProjectCard setSearchInput={setSearchInput} key={key} project={project}/>
                  )
                })}
